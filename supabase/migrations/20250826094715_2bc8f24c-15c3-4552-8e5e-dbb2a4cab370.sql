...
ADD CONSTRAINT unique_shotguner_id UNIQUE (shotguner_id);

-- Create indexes for performance
CREATE INDEX idx_wheel_spins_shotguner_id ON public.wheel_spins (shotguner_id);
CREATE INDEX idx_wheel_spins_shotguner_email ON public.wheel_spins (shotguner_email);
CREATE INDEX idx_wheel_spins_spinned_at ON public.wheel_spins (wheel_spinned_at);
...
CREATE POLICY "Backend only access to wheel_spins" 
